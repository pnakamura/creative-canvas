...
import { useFlowStore, NodeData, DocumentSettings } from '@/store/flowStore';
...
const templateLabels: Record<string, { label: string; icon: React.ElementType }> = {
  memo: { label: 'Memo', icon: Mail },
  letter: { label: 'Letter', icon: FileSignature },
  policy: { label: 'Policy', icon: FileWarning },
  procedure: { label: 'Procedure', icon: ScrollText },
  contract: { label: 'Contract', icon: FileKey },
  custom: { label: 'Custom', icon: FileCheck },
};
...
export const DocumentGeneratorNode: React.FC<NodeProps> = (props) => {
  const { updateNodeData, getConnectedNodes } = useFlowStore();
  const nodeData = props.data as NodeData;
  const { isProcessing, error, generatedOutput, documentSettings } = nodeData;
  const { toast } = useToast();

  const settings: DocumentSettings = documentSettings || {
    style: 'business',
    format: 'markdown',
    includeHeader: true,
    includeFooter: true,
  ...
  const TemplateIcon = templateLabels[settings.templateType]?.icon || FileCheck;
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${templateLabels[settings.templateType]?.label} created successfully`,
      });
    } catch (err) {
  ...
  return (
    <BaseNode
      ...
      fixedDescription="Generate official documents"
      nodeCategory="output"
    ...
    >
      <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          <Badge variant="outline" className={cn('text-[10px] gap-1 bg-green-500/20 text-green-400')}>
            <TemplateIcon className="w-2.5 h-2.5" />
            {templateLabels[settings.templateType]?.label}
          </Badge>
        ...
        <Button
          ...
          className={cn(
            'nodrag w-full gap-2 bg-green-500/20 hover:bg-green-500/30 text-green-400 border border-green-500/30',
